...
public class clientGUI extends JFrame implements Runnable, ActionListener,KeyListener {	
	...
	/*
	 * initComponents()
	 * Initialize client game components before game start
	 */
		...
		try{
			...
			while(true){
				...
				if(text.startsWith("#!")){
					...
					if(text.equals("READY")){				// server is ready to play
						game.hostReady = true;
						...
						if(text.charAt(0)=='H'||text.charAt(0)=='M'){
							...
							if(myShipGrid.checkForHit(y, x)){			// check if there is a ship at this position
								game.playSound(2);						// play hit sound
								placePeg('h',y,x);						// red peg
	...
	private void placePeg(char s, int x, int y) {
		Icon ico;
		if(s == 'h'){
			ico = (Icon) new ImageIcon(Constants.REDPEG);			// set ico as red peg
		}else{
			ico = (Icon) new ImageIcon(Constants.WHITEPEG);			// set ico as white peg
		}
		...
		smallGrid.add(peg, 0);										// place peg on the board
	...
	static private void whatDidYouSink(String msg){			// display the ship that was sunk
		youSunk.setText(msg);
	}
